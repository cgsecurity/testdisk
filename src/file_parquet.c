...
const file_hint_t file_hint_parquet= {
  ...
  .recover=1,
  .enable_by_default=1,
  .register_header_check=&register_header_check_parquet
};
...
{
  /* Magic "PAR1" at offset 0 */
  reset_file_recovery(file_recovery_new);
  file_recovery_new->extension=file_hint_parquet.extension;
  return 1;
}

static void register_header_check_parquet(file_stat_t *file_stat)
{
  static const unsigned char parquet_magic[4]= { 'P', 'A', 'R', '1' };
  register_header_check(0, parquet_magic, sizeof(parquet_magic), &header_check_parquet, file_stat);
}
#endif
